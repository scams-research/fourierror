...
def dft(
    ...
    f_samples = (2 / data.values.size) * (
        dft_matrix @ mv_norm.rvs(n_samples, random_state=random_state).T
    )
...
def fft(
    ...
    f_samples = (2 / data.values.size) * np.fft.fft(
        mv_norm.rvs(n_samples, random_state=random_state)
    ).T
